...
def parse_xml(adf_xml):
    ...
    try:
        ...
        log.info('xml parse successful.')
    except Exception as e:
        log.error(f'xml parse error due to {e}')
    return obj
...
def validate_adf_values(input_json):
    ...
    if not first_name or not last_name:
        if not first_name:
            log.info('first_name missing field in input_json')
        if not last_name:
            log.info('last_name missing field in input_json')
        return {"status": "REJECTED", "code": "6_MISSING_FIELD", "message": "name is incomplete"}

    if not email and not phone:
        if not email:
            log.info('email missing field in input_json')
        if not phone:
            log.info('phone missing field in input_json')
    ...
    if not res:
        log.info('Invalid postal code in input_json')
    ...
    for id in input_json['id']:
        if id['@source'] == 'TCPA_Consent' and id['#text'].lower() == 'yes':
            log.info(f'tcpa_consent is true for id: {id}')
            tcpa_consent = True
    if not email and not tcpa_consent:
        log.info(f'input_json missing email or doesnot have tcpa_consent')
    ...
    if not validate_iso8601(input_json['requestdate']):
        log.info('input_json has missing or invalid requestdate')
...
def check_validation(input_json):
    try:
        ...
        log.info('input_json validation check')
        if response['status'] == "REJECTED":
            log.info('input_json validation rejected')
